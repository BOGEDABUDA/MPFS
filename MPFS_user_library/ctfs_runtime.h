...
#define CT_PATH2INODE_HASH_SIZE 10240
#define CT_PATH2INODE_HASH_OPEN_ADDRESS 10
/* File descriptor
 */
struct ct_fd_t
{
...
struct ct_runtime
{
  ...
  // pointer to path2inode hash table, stored in the CT_OFFSET_PATH_HASH
  ct_inode_pt path2inode[CT_PATH2INODE_HASH_SIZE];
  pthread_spinlock_t* path2inode_lock;
...
#define CT_INODE_FRAME_SAME_INODE_LOCK                                         \
  0b00100  // set if current and parent share the same lock
#define CT_INODE_FRAME_INSTALL                                                 \
  0b01000  // install the current inode to the given location. No lock held when
           // return.
