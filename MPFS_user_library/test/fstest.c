...
#define DEFAULT_SIZE                                                           \
  (2 * 1024 * 1024 * (unsigned long)1024)  //(128*1024*(unsigned long)1024) //
...
int main(int argc, char* argv[])
{
  ...
  while ((opt = getopt(argc, argv, "wrRaSs:n:G:M:K:p:F")) != -1)
  {
    switch (opt)
    {
      case 'w': {
        _wr = 1;
        printf("command for executing write test\n");
        break;
      }
      case 'r': {
        _rd = 1;
  ...
  else
  {
    ...
    {
      sprintf(path_buf, "%s", dest);
      printf("executing write test from sequential_rdwr\n");
  ...
  printf("time used: %ld ns\nTotal Written: %ld\nTotal Read: %ld\nWrite speed: "
         "%f GB/s\nRead speed: %f GB/s\n",
         total_nsec, total_writen, total_read,
         (double)total_writen / (double)total_nsec,
         (double)total_read / (double)total_nsec);
  printf("\tNomalized 2GB time(W/R): %ld ns / %ld ns\n",
         (unsigned long)((double)total_nsec / (double)total_writen *
                         (double)DEFAULT_SIZE),
         (unsigned long)((double)total_nsec / (double)total_read *
                         (double)DEFAULT_SIZE));
}
...
void sequential_rdwr(const char* path, unsigned long size)
{
  ...
  if (_wr)
  {
    ...
    while (1)
    {
      ...
      // long)((double)time_diff / (double)res * (double)DEFAULT_SIZE));
      print_debug(fd_wr);
      total_writen += res;
...
void small_chunck_rdwr(const char* path, unsigned long size)
{
  ...
  if (_wr)
  {
    ...
    printf("Writing buffer to disk end: %ld\n", res);
    printf("time used: %ld ns\nspeed: %f GB/s\n", time_diff,
           (double)res / (double)time_diff);
    printf(
      "\tNomalized 2GB time: %ld ns\n",
