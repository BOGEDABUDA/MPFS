...
const uint64_t pgg_size[10] = {CT_PGGSIZE_LV0, CT_PGGSIZE_LV1, CT_PGGSIZE_LV2,
                               CT_PGGSIZE_LV3, CT_PGGSIZE_LV4, CT_PGGSIZE_LV5,
                               CT_PGGSIZE_LV6, CT_PGGSIZE_LV7, CT_PGGSIZE_LV8,
                               CT_PGGSIZE_LV9};

const uint64_t pgg_limit[10] = {
  CT_LIMITSIZE_LV0, CT_LIMITSIZE_LV1, CT_LIMITSIZE_LV2, CT_LIMITSIZE_LV3,
  CT_LIMITSIZE_LV4, CT_LIMITSIZE_LV5, CT_LIMITSIZE_LV6, CT_LIMITSIZE_LV7,
  CT_LIMITSIZE_LV8, CT_LIMITSIZE_LV9};

const uint64_t pgg_subpmd_count_per_pkg[3] = {512, 64, 8};
...
void pgg_alloc_prot_file_add(pgg_header_pt header, relptr_t target)
{
  int64_t index;
retry:
  index = find_free_bit(&ct_rt.super_blk->alloc_prot_bmp, CT_ALLOC_PROT_SIZE,
                        ct_rt.super_blk->alloc_prot_clock + 1);
  if (index == -1)
  {
    goto retry;
  }
...
void __pgg_build_sub_pmd(pgg_header_pt parent, uint8_t index, pgg_level_t level)
{
  ...
  pgg_hd_group_pt target_group =
    (pgg_hd_group_pt)((uint64_t)parent_group + (index * pgg_size[3]));
...
void __pgg_update_cap_alloc(pgg_header_pt header)
{
  ...
  while (current_lvl != 9)
  {
    ...
    // check the maximum capability level of all 8 blocks (including first sub
    // ppg)
    for (uint16_t i = 0; i < 8; i++)
    {
      if (PGG_STATE_LOAD(current_header->state_map, i) == PGG_STATE_EMPTY)
      {
        // if any of the 8 blocks is empty, level - 1 is definitely avaliable,
        // no change is required
        return;
      }
      else if (current_lvl != PGG_LVL4 &&
               PGG_STATE_LOAD(current_header->state_map, i) == PGG_STATE_SUB)
      {
        // if the block is used as sub ppg, we check the capability and pass to
        // parent
        pgg_header_pt child = PGG_GROUP_AT2HEADER(
          PGG_HEADER2GROUP(current_header), current_header->level - 1, i);
...
/* Build new sub page group. (Partitioned partition)
 ...
 * @param[in] level
 */
void __pgg_new_subpgg(pgg_header_pt header,
                      pgg_header_pt parent_header,
                      pgg_level_t level)
{
  assert(level <= 9);
  pgg_hd_group_pt group = PGG_HEADER2GROUP(header);
  /* create sub-pmd package */
  /* Fill all availiable headers (<=level) in the header group */
...
{
  // if the allocated level is just the child level of header
  if (level + 1 == header->level)
  {
    ...
    {
      // achieve the state of a specific partition and check if it's empty
  ...
  {  // if the allocated level is less than the child level of header
     // we need to go deeper into the headers of lower levels
    uint16_t map = header->state_map;
    for (uint16_t i = 0; i < 8; i++)
    {
      // find a partition marked as partitioned
      if (PGG_STATE_LOAD(map, i) == PGG_STATE_SUB)
      {
        // achieve the pointer to a partitioned child header
        pgg_header_pt child =
          PGG_GROUP_AT2HEADER(PGG_HEADER2GROUP(header), header->level - 1, i);
        ...
        // begin to recursively allocate until the child level matches the
        // allocated level
        return __pgg_allocate_big(level, child);
      }
    }
    // no existing Partitioned partition available. create a new one from Empty.
    for (uint16_t i = 1; i < 8; i++)
    {
      if (PGG_STATE_LOAD(map, i) == PGG_STATE_EMPTY)
      {
        ...
        pgg_header_pt child =
          PGG_GROUP_AT2HEADER(PGG_HEADER2GROUP(header), child_lvl, i);
        __pgg_new_subpgg(child, header, child_lvl);  //! BUG
        assert(child->level == header->level - 1);
...
relptr_t __pgg_sub_pmd_alloc(pgg_level_t level, pgg_subpmd_header_pt header)
{
  ...
  int64_t target =
    (level == PGG_LVL2) ?
      find_free_bit_tiny(header->bitmap, pgg_subpmd_count_per_pkg[level]) :
      find_free_bit(header->bitmap, pgg_subpmd_count_per_pkg[level],
                    header->bitmap_hint);
...
relptr_t __pgg_allocate_small(pgg_level_t level, pgg_header_pt header)
{
  ...
  int found;
  //? test whether the current header can directly allocate the target-level
  // page.
  if (PGG_SUBPMD_PRF_TEST(header->sub_pmd_cap, level))
  {
    ...
    while (1)
    {
      // iterate each layer
      found = 0;
      //? group is a collection of 6 six headers for L4-L9
      group = PGG_HEADER2GROUP(header);
#ifdef DAX_DEBUGGING
      pgg_hd_group_t gr = *group;
#endif
      //? 8 is the number of sub-level in a parent level
      for (uint16_t i = 0; i < 8; i++)
      {
  ...
  //? test whether the current header can allocate a lvl3 page, which could be
  //? divided into lower-level pages
  ...
  if (target->taken == pgg_subpmd_count_per_pkg[level])
  {
    ...
    for (uint16_t i = 0; i < 8; i++)
    {
      if (PGG_STATE_LOAD(current->state_map, i) == PGG_STATE_SUB)
      {
        ...
        if (sp_header->level == level &&
            sp_header->taken != pgg_subpmd_count_per_pkg[level])
        {
          // still space left
          // stay same
          need_act = 0;
...
{
  relptr_t ret;
  bitlock_acquire(&ct_rt.super_blk->pgg_lock, 0);
  if (level <= PGG_LVL2)
  {  // allocate less than 2MB pgg
    ret = __pgg_allocate_small(level, &ct_rt.first_pgg->header[0]);
#if CTFS_DEBUG > 0
    printf("\tallocated lvl %d @0x%lx\n", ret);
#endif
    bitlock_release(&ct_rt.super_blk->pgg_lock, 0);
    return ret;
  }
  else if (level > PGG_LVL2 && level <= PGG_LVL9)
  {  // allocate more than 2MB pgg
...
void pgg_deallocate(pgg_level_t level, relptr_t target)
{
  ...
  {
    // PMD and above
    pgg_header_pt header =
      &PGG_REL2HD_GROUP(target, level + 1)->header[9 - (level + 1)];
#ifdef CTFS_DEBUG
  ...
  {
    // sub PMD
    pgg_subpmd_header_pt header =
      &PGG_REL2HD_GROUP(target, PGG_LVL3)->subpmd_header;
