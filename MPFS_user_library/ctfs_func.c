...
static inline void*
intel_memcpy(void* __restrict__ b, const void* __restrict__ a, size_t n)
{
  char* s1       = b;
  const char* s2 = a;
  for (; 0 < n; --n)
...
int ctfs_mkfs(int flag)
{
  ...
  //! MPFS-specific
  printf("MPFS page table sharing!\n");

  struct timespec start, end;
  clock_gettime(CLOCK_MONOTONIC, &start);
  ioctl(dax_fd, MPFS_IOCTL_PTSHARE, ct_rt.base_addr);

  clock_gettime(CLOCK_MONOTONIC, &end);
  long runtime1 =
    (end.tv_sec - start.tv_sec) * 1000000000 + (end.tv_nsec - start.tv_nsec);
  printf("Runtime of MPFS_IOCTL_PTSHARE: %ld ns\n", runtime1);

  clock_gettime(CLOCK_MONOTONIC, &start);
  ioctl(dax_fd, MPFS_IOCTL_ENABLE_WRITE);
  clock_gettime(CLOCK_MONOTONIC, &end);
  long runtime2 =
    (end.tv_sec - start.tv_sec) * 1000000000 + (end.tv_nsec - start.tv_nsec);
  printf("Runtime of MPFS_IOCTL_ENABLE_WRITE: %ld ns\n", runtime2);
#endif
  ...
  strcpy(sb->magic, CT_MAGIC);  //! bug here, trigger kernel panic
  // dax_stop_write(ct_rt.mpk[DAX_MPK_DEFAULT]);

  // init the super block, which is always stored in the beginning of virtual
  // address space. other process can access it by sharing PM mapping and
  // reading the beginning. super block stores the relative address of some
  // important data structures of MPFS
  ...
  // get the absolute address of the data structures via the relative address
  ct_rt.alloc_prot  = CT_REL2ABS(sb->alloc_prot_bmp);
  ct_rt.lvl9_bmp    = CT_REL2ABS(sb->lvl9_bmp);    // never used
  ct_rt.first_pgg   = CT_REL2ABS(sb->first_pgg);   // address of first pgg
  ct_rt.inode_bmp   = CT_REL2ABS(CT_OFFSET_IBMP);  // address of inode bitmap
  ct_rt.inode_start = CT_REL2ABS(CT_OFFSET_ITABLE);
  // ct_rt.path2inode  = (ct_inode_pt*)CT_REL2ABS(CT_OFFSET_PATH_HASH);
  // ct_rt.path2inode_lock =
  //   CT_REL2ABS(CT_OFFSET_PATH_HASH_LOCK);  // lock for path2inode
  // memset((void*)ct_rt.path2inode, 0,
  //        ((uint64_t)1 << 20));  // reset the hash table while mkfs
  // pthread_spin_init(ct_rt.path2inode_lock, PTHREAD_PROCESS_PRIVATE);
  //! reset all locks to prevent permanent faults
  printf("MPFS shared lock inited\n");
  ctfs_lock_init(
    ct_rt.super_blk
      ->open_lock);  // lock for open to avoid race condition in getting fd and
                     // parsing path to achieving inode
  ctfs_lock_init(ct_rt.super_blk->inode_bmp_lock);  // lock for inode bitmap
  ctfs_lock_init(ct_rt.super_blk->open_lock);
  ct_rt.super_blk->pgg_lock = 0;
  for (int i = 0; i < (CT_INODE_BITLOCK_SLOTS / 64); i++)
  {
    ct_rt.super_blk->inode_rt_lock[i] = 0;
    ct_rt.super_blk->inode_rw_lock[i] = 0;
  }
  // allocate inode
  ...
  // printf("root inode: %lu, %p\n", ct_rt.super_blk->root_inode,
  //        &ct_rt.super_blk->root_inode);
...
//! MPFS
// detect whether the main_task has been created
// if created, sharing its page table by invoking prefault, if not, create a new
// one
int mpfs_init()
{
  MPFS_PRINT("********MPFS Enabled********\n");
  MPFS_PRINT("MPFS page table sharing!\n");
  ...
  long runtime1 =
    (end.tv_sec - start.tv_sec) * 1000000000 + (end.tv_nsec - start.tv_nsec);
  printf("MPFS_init: Runtime of MPFS_IOCTL_PTSHARE: %ld ns\n", runtime1);
  ...
  long runtime2 =
    (end.tv_sec - start.tv_sec) * 1000000000 + (end.tv_nsec - start.tv_nsec);
  printf("MPFS_init: Runtime of MPFS_IOCTL_ENABLE_WRITE: %ld ns\n", runtime2);
  return 0;
}

int mpfs_generate_page_table_by_prefault()
{
  char* addr = NULL;
  // the constructor function of ctfs has mapped the dax device.
  // access the first entries of the 4 PGD-aligned region to trigger the page
  // fault
  for (int i = 0; i < MPFS_PHYS_RES / PGD_SIZE; i++)
  {
    addr  = (char*)ct_rt.base_addr + i * PGD_SIZE;
    *addr = 0;
  }
  return 0;
}

int mpfs_init_main_task()
{
  long ret = ioctl(dax_fd, MPFS_IOCTL_MAINTASK, ct_rt.base_addr);
  return ret;
}

// due to some logic bugs, we have to update the vstart of main task with such a
// individual function
int mpfs_update_vstart_for_main_task()
{
  long ret = ioctl(dax_fd, MPFS_IOCTL_VSTART, ct_rt.base_addr);
  return ret;
}
...
int ctfs_init_for_main_task(int flag)
{
  ...
  mpfs_update_vstart_for_main_task();  // update vstart and rebuild page table
                                       // if not reset

  mpfs_generate_page_table_by_prefault();  // build the subtree of page table
  // for MPFS

  mpfs_init_main_task();  // record the four PGD entries of the subtree (root of
  // subtree)
  ...
  ctfs_lock_init(ct_rt.super_blk->open_lock);
  ctfs_lock_init(ct_rt.super_blk->inode_bmp_lock);
  ct_rt.super_blk->pgg_lock = 0;
...
int ctfs_init(int flag)
{
  ...
  dax_ioctl_init_t frame = {.size = CT_DAX_ALLOC_SIZE};
  ct_rt.base_addr        = (uint64_t)dax_start("/dev/dax0.0", &frame);
  // install the shared page table before setup the ctfs to avoid duplicated
  // page fault.
  MPFS_PRINT("mpfs initializing...\n");
  mpfs_init();
  MPFS_PRINT("mpfs initialized...\n");
  ...
  dax_grant_access(ct_rt.mpk[DAX_MPK_DEFAULT]);
  assert(strcmp(ct_super->magic, CT_MAGIC) == 0);
  // get the absolute address of MPFS runtime via the relative address stored in
  // super block, it has already been initialized in ctfs_mkfs while executing
  // './mkfs'
  ct_super_blk_pt sb  = ct_rt.super_blk;
  ct_rt.alloc_prot    = CT_REL2ABS(sb->alloc_prot_bmp);
  ct_rt.lvl9_bmp      = CT_REL2ABS(sb->lvl9_bmp);
...
int ctfs_open(const char* pathname, int flags, ...)
{
  ...
  dax_grant_access(ct_rt.mpk[DAX_MPK_DEFAULT]);
  ctfs_lock_acquire(ct_rt.super_blk->open_lock);
#ifdef CTFS_DEBUG
  printf("****opening %s ******\n", pathname);
#endif
  if (flags & O_CREAT)
  ...
  // find a free fd
  for (fd = ct_rt.next_fd; fd < CT_MAX_FD; fd++)
  {
    if (ct_rt.fd[fd].inode == NULL)
    {
      ct_rt.next_fd = fd + 1;  // update next fd
      if (ct_rt.next_fd == CT_MAX_FD)
        ct_rt.next_fd = 0;
      break;
    }
  }
  ...
  {
    ct_rt.errorn = ENFILE;
    ctfs_lock_release(ct_rt.super_blk->open_lock);
    dax_stop_access(ct_rt.mpk[DAX_MPK_DEFAULT]);
#ifdef CTFS_DEBUG
...
#endif
  read_lock(&ct_rt.fd[fd].inode->i_rwlock);
  if (offset >= ct_rt.fd[fd].inode->i_size)
  {
    printf("offset: %lu, size: %lu\n", offset, ct_rt.fd[fd].inode->i_size);
    read_unlock(&ct_rt.fd[fd].inode->i_rwlock);
    dax_stop_access(ct_rt.mpk[DAX_MPK_DEFAULT]);
    return 0;
  }
...
ctfs_pwrite_normal(int fd, const void* buf, size_t count, off_t offset)
{
  ...
  ct_inode_t ino = *ct_rt.fd[fd].inode;
#endif
  write_lock(&ct_rt.fd[fd].inode->i_rwlock);
  ...
  if (unlikely(end > ct_rt.fd[fd].inode->i_size))
  {
    ...
    if (inode_resize(ct_rt.fd[fd].inode, offset + count))  //! pswap here
    {
      ct_rt.fd[fd].prefaulted_bytes = 0;
    }
  ...
  ct_rt.fd[fd].cpy_time += timer_end();
#endif
  write_unlock(&ct_rt.fd[fd].inode->i_rwlock);
  dax_stop_access(ct_rt.mpk[DAX_MPK_DEFAULT]);

  return count;
}

static inline void ctfs_pwrite_atomic_cpy(ct_inode_pt inode,
                                          void* base,
                                          void* staging,
                                          const void* buf,
                                          size_t count,
...
static inline ssize_t
ctfs_pwrite_atomic(int fd, const void* buf, size_t count, off_t offset)
{
  if (unlikely(fd >= CT_MAX_FD || ct_rt.fd[fd].inode == NULL))
  {
    ct_rt.errorn = EBADF;
  ...
  // if(ct_rt.fd[fd].inode->i_level == PGG_LVL_NONE || offset >=
  // ct_rt.fd[fd].inode->i_size){
  ...
  if (count <= 4096 * 4)
  {
    ctfs_pwrite_atomic_cpy(ct_rt.fd[fd].inode, base, staging, buf, count,
                           offset);
    goto out;
  }
#endif
  ...
  // lock
  // bitlock_acquire(&ct_rt.super_blk->pgg_lock, 32);
  ...
  if (residue != 0)
  {
    ...
    if (rem < CT_PAGE_SIZE - residue)
    {
      ...
      {
        avx_cpy(staging + cur + residue + rem, base + cur + residue + rem,
                CT_PAGE_SIZE - residue - rem);
      }
      swap_num = 1;
      goto end_cpy;
    }
...
end_cpy:
  frame =
    (dax_ioctl_pswap_t){.ufirst  = base + swap_start,
  ...
  if (offset != 0)
  {
    ...
    {
      assert(memcmp(base + swap_start, staging + swap_start,
                    offset - swap_start) == 0);
    }
...
out:
  ...
  // bitlock_release(&ct_rt.super_blk->pgg_lock, 32);
...
int ctfs_rename(const char* oldpath, const char* newpath)
{
  ...
  // link new
  frame =
    (ct_inode_frame_t){.flag = CT_INODE_FRAME_INSTALL | CT_INODE_FRAME_CREATE,
                       .path = newpath,
                       .current = target};
  if (*newpath != '/')
  {
...
int ctfs_lseek(int fd, int offset, int whence)
{
  ...
  switch (whence)
  {
    case SEEK_SET: ct_rt.fd[fd].offset = offset; break;
