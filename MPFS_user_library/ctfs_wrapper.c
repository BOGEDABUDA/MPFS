...
OP_DEFINE(OPEN)
{
  if (real_ops.OPEN == NULL)
  {
    ...
    if (real_ops.OPEN == NULL)
    {
      perror("Fail to load sys call");
    }
    else
    {
      printf("Load sys call successfully\n");
    }
  }
...
OP_DEFINE(FCNTL)
{
  ...
  else
  {
    ...
    {
      case F_GETFD:
      case F_GETFL: return real_ops.FCNTL(fd, cmd); break;
      case F_SETFD:
      case F_SETFL: return real_ops.FCNTL(fd, cmd, va_arg(ap, int));
      case F_SET_RW_HINT:
        return real_ops.FCNTL(fd, cmd, va_arg(ap, void*));
        break;
      case F_SETLK:
        return real_ops.FCNTL(fd, cmd, va_arg(ap, struct flock*));
        break;
