...
void dax_stop_write(int key)
{
  ...
  // ioctl(dax_fd, MPFS_IOCTL_DISABLE_WRITE);
}

void dax_grant_access(int key)
{
  // ioctl(dax_fd, MPFS_IOCTL_ENABLE_PTE_WRITE);
  pkey_set(key, 0);
}
...
long dax_pswap(dax_ioctl_pswap_t* frame)
{
  ...
  printf("PSWAP Runtime: %.6f seconds, npgs: %ld \n", elapsed_time, frame->npgs);
...
void* dax_start(const char* path, dax_ioctl_init_t* frame)
{
  ...
  // if(!dax_ready()){
  // dax_ret = ioctl(dax_fd, MPFS_IOCTL_SHOWVMA, (uint64_t)ret);
