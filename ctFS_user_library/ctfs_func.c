...
#  ifdef CTFS_DEBUG
  printf("mpk value: default: %d, file: %d, meta: %d\n",
         ct_rt.mpk[DAX_MPK_DEFAULT], ct_rt.mpk[DAX_MPK_FILE],
...
ctfs_pwrite_normal(int fd, const void* buf, size_t count, off_t offset)
{
  ...
  // dax_sync_write_lock();  // lock all page tables and dax page table
  ...
  // dax_sync_write_unlock();  // unlock all page tables and dax page table
