...
enum dax_ioctl_types
{
  ...
  DAX_IOCTL_COW,
  DAX_IOCTL_COPYTEST,
  DAX_IOCTL_PSWAP_DRAM,
  DAX_SYNC_WRITE_LOCK,
  DAX_SYNC_WRITE_UNLOCK,
  DAX_SYNC_READ_LOCK,
...
{
  dax_ioctl_pswap_t dax_frame;
  uint64_t from_id;
};
typedef struct frame_wrapper frame_wrapper_t;
...
int generate_msgid(uint64_t process_id);

void dax_IPC_terminate_msg();

void dax_IPC_send_message(dax_ioctl_pswap_t* frame, uint64_t process_id);

void dax_IPC_receive_message();

long dax_pswap(dax_ioctl_pswap_t* frame);

long DRAM_pswap(dax_ioctl_pswap_t* frame);
