...
#define PGG_SUBPMD_AVB_TEST(cap, lvl) (cap & (0b01 << lvl))
#define PGG_SUBPMD_PRF_TEST(cap, lvl) (cap & (0b010000 << lvl))
...
#define PGG_STATE_EMPTY 0x00  // Empty
#define PGG_STATE_FILE 0x01   // Allocated
#define PGG_STATE_SUB 0x02    // Partitioned

#define PGG_STATE_INIT PGG_STATE_SUB

/* conversions */
#define PGG_STATE_LOAD(map, pos) (0b011 & (map >> (2 * (pos))))
#define PGG_STATE_STORE(map, pos, val)                                         \
  map = ((((val & 0b011) << (2 * (pos))) | map) &                              \
         ((val << (2 * (pos))) | ~(0b011 << (2 * (pos)))))

#define PGG_REL2HD_GROUP(addr, level)                                          \
  ((pgg_hd_group_pt)CT_REL2ABS(addr & (PAGE_MASK << (3 * (level)))))
#define PGG_BIGFILE2INDEX(addr, level) ((addr >> (12 + 3 * (level))) & 0b0111)
...
struct pgg_header
{
   ...
   * 4-bit preferred + 4-bit available.(L0-L3)
   */
  uint8_t sub_pmd_cap;
...
#define CT_ALLOC_PROT_PTR                                                      \
  ((ct_alloc_prot_pt)(((void*)(ct_rt.first_pgg)) + 1024))
