...
static void fallocate_file(struct thread_data* td, struct fio_file* f)
{
  ...
  {
    case FIO_FALLOCATE_NATIVE: native_fallocate(td, f); break;
    case FIO_FALLOCATE_NONE: break;
...
static int extend_file(struct thread_data* td, struct fio_file* f)
{
   ...
   */
  if (td_read(td) || (td_write(td) && td->o.overwrite && !td->o.file_append) ||
...
static bool pre_read_file(struct thread_data* td, struct fio_file* f)
{
  ...
  if (td_ioengine_flagged(td, FIO_PIPEIO) || td_ioengine_flagged(td, FIO_NOIO))
    return true;
...
static int __file_invalidate_cache(struct thread_data* td,
                                   struct fio_file* f,
                                   ...
                                   unsigned long long len)
{
   ...
   * continue on our way.
   */
  //! MPFS will return a bad fd, so we should comment this
  // if (errval)
...
int file_lookup_open(struct fio_file* f, int flags)
{
  ...
  // f->fd = MPFS_OPEN(f->file_name, flags, 0600);
  return from_hash;
}
...
uint64_t get_start_offset(struct thread_data* td, struct fio_file* f)
{
  ...
  else
  {
    ...
    offset = o->start_offset + td->subjob_number * increment;
  }
...
int setup_files(struct thread_data* td)
{
  ...
  for_each_file(td, f, i)
  {
    if (!td_ioengine_flagged(td, FIO_DISKLESSIO) &&
        ...
        !(td->flags & TD_F_DIRS_CREATED) && !create_work_dirs(td, f->file_name))
      goto err_out;
  }
  ...
  for_each_file(td, f, i)
  {
    ...
    else
    {
      ...
      if (o->size_percent && o->size_percent != 100)
      {
        ...
        f->io_size = (file_size * o->size_percent) / 100;
    ...
    {
      if (!td_ioengine_flagged(td, FIO_DISKLESSIO) && !o->create_on_open)
      {
        need_extend++;
  ...
  if (td->o.block_error_hist)
  {
    ...
    for (i = 0; i < len; i++)
      td->ts.block_infos[i] = BLOCK_INFO(0, BLOCK_STATE_UNINIT);
  }
  else
  ...
  if (need_extend)
  {
    ...
    if (output_format & FIO_OUTPUT_NORMAL)
    {
      log_info("%s: Laying out IO file%s (%u file%s / %s%lluMiB)\n", o->name,
               ...
               need_extend > 1 ? "s" : "", need_extend > 1 ? "total " : "",
               extend_size >> 20);
    }
    ...
    for_each_file(td, f, i)
    {
      ...
      err = __file_invalidate_cache(td, f, old_len, extend_len);
  ...
  if (td->io_ops->prepopulate_file)
  {
    ...
    for_each_file(td, f, i)
    {
      ...
      err = __file_invalidate_cache(td, f, f->file_offset, f->io_size);
...
static void __init_rand_distribution(struct thread_data* td, struct fio_file* f)
{
  ...
  if (td->o.random_distribution == FIO_RAND_DIST_ZIPF)
    zipf_init(&f->zipf, nranges, td->o.zipf_theta.u.f, td->o.random_center.u.f,
              seed);
  else if (td->o.random_distribution == FIO_RAND_DIST_PARETO)
    pareto_init(&f->zipf, nranges, td->o.pareto_h.u.f, td->o.random_center.u.f,
                seed);
  else if (td->o.random_distribution == FIO_RAND_DIST_GAUSS)
    gauss_init(&f->gauss, nranges, td->o.gauss_dev.u.f, td->o.random_center.u.f,
               seed);
}
...
static bool init_rand_distribution(struct thread_data* td)
{
  ...
  for_each_file(td, f, i) __init_rand_distribution(td, f);
 ...
 * 64-bit capable.
 */
static int check_rand_gen_limits(struct thread_data* td,
                                 struct fio_file* f,
                                 uint64_t blocks)
{
...
int add_file(struct thread_data* td, const char* fname, int numjob, int inc)
{
  ...
  {
    case FILE_LOCK_NONE: break;
    case FILE_LOCK_READWRITE: f->rwlock = fio_rwlock_init(); break;
    case FILE_LOCK_EXCLUSIVE: f->lock = fio_sem_init(FIO_SEM_UNLOCKED); break;
    default:
...
int get_fileno(struct thread_data* td, const char* fname)
{
  ...
  for_each_file(td, f, i) if (!strcmp(f->file_name, fname)) return i;
...
bool fio_files_done(struct thread_data* td)
{
  ...
  for_each_file(td, f, i) if (!fio_file_done(f)) return false;
...
#  if defined(__sun__)
    if (ret == ENOTTY)
    { /* ENOTTY suggests RAW device or ZFS */
