...
static int fio_syncio_prep(struct thread_data* td, struct io_u* io_u)
{
  ...
  {
    // if (MPFS_SEEK(f->fd, io_u->offset, SEEK_SET) == -1)
    // {
                                          ...
                                          struct io_u* io_u)
{
  ...
  // ret = MPFS_READ(f->fd, io_u->xfer_buf, io_u->xfer_buflen);
  else if (io_u->ddir == DDIR_WRITE)
    ret = write(f->fd, io_u->xfer_buf, io_u->xfer_buflen);
  // ret = MPFS_WRITE(f->fd, io_u->xfer_buf, io_u->xfer_buflen);
  else if (io_u->ddir == DDIR_TRIM)
  {
    do_io_u_trim(td, io_u);
    return FIO_Q_COMPLETED;
...
static int fio_vsyncio_getevents(struct thread_data* td,
                                 unsigned int min,
                                 unsigned int max,
...
static void
fio_vsyncio_set_iov(struct syncio_data* sd, struct io_u* io_u, int idx)
{
  sd->io_us[idx]           = io_u;
