...
#include "../ctfs.h"       //! MPFS added for invoking ctFS
#include "../mpfs_main_task.h"  //! MPFS added
#include "filebench.h"
#include "fsplug.h"
#ifdef HAVE_AIO
#  include <aio.h>
 ...
 */
static aiolist_t* aio_allocate(flowop_t* flowop)
{
  aiolist_t* aiolist;
 ...
 */
static int aio_deallocate(flowop_t* flowop, struct aiocb64* aiocb)
{
...
static int fb_lfsflow_aiowrite(threadflow_t* threadflow, flowop_t* flowop)
{
  ...
  if (avd_get_bool(flowop->fo_random))
  {
    ...
    if (wss < iosize)
    {
      filebench_log(LOG_ERROR, "file size smaller than IO size for thread %s",
                    flowop->fo_name);
      return (FILEBENCH_ERROR);
    }
...
#  ifdef HAVE_AIOWAITN
    if (((aio_waitn64((struct aiocb64**)worklist, MAXREAP, &todo, &timeout)) ==
         -1) &&
        errno && (errno != ETIME))
    {
...
#ifdef MPFS_SPECIFIC
static int fb_lfs_open(fb_fdesc_t* fd, char* path, int flags, int perms)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: open:%s\n", path);
#  endif
  if ((fd->fd_num = MPFS_OPEN(path, flags, perms)) < 0)
    return (FILEBENCH_ERROR);
  else
    return (FILEBENCH_OK);
}
 ...
 */
static int fb_lfs_unlink(char* path)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: unlink:%s\n", path);
#  endif
  return (MPFS_UNLINK(path));
}

/*
 * Does a readlink of a symbolic link.
 */
static ssize_t fb_lfs_readlink(const char* path, char* buf, size_t buf_size)
{
  printf("ERROR: readlink not implemented\n");
 ...
 */
static int fb_lfs_fsync(fb_fdesc_t* fd)
{
  printf("ERROR: fsync not implemented\n");
 ...
 */
static int fb_lfs_lseek(fb_fdesc_t* fd, off64_t offset, int whence)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: lseek\n");
#  endif
  return (MPFS_SEEK(fd->fd_num, offset, whence));
}

/*
 * Do a posix rename of a file. Return what rename() returns.
 */
static int fb_lfs_rename(const char* old, const char* new)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: rename:%s-->%s\n", old, new);
#  endif
  return (MPFS_RENAME(old, new));
}

/*
 * Do a posix close of a file. Return what close() returns.
 */
static int fb_lfs_close(fb_fdesc_t* fd)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: close:%d\n", fd->fd_num);
#  endif
  return (MPFS_CLOSE(fd->fd_num));
}

/*
 * Use mkdir to create a directory.
 */
static int fb_lfs_mkdir(char* path, int perm)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: mkdir:%s\n", path);
#  endif
  return (MPFS_MKDIR(path, perm));
}

/*
 * Use rmdir to delete a directory. Returns what rmdir() returns.
 */
static int fb_lfs_rmdir(char* path)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: rmdir:%s\n", path);
#  endif
  return (MPFS_RMDIR(path));
}
 ...
 */
static void fb_lfs_recur_rm(char* path)
{
  int ret;
  char cmd[MAXPATHLEN];
 ...
 * NULL on failure.
 */
static DIR* fb_lfs_opendir(char* path)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: opendir:%s\n", path);
#  endif
  return (MPFS_OPENDIR(path));
}
 ...
 */
static struct dirent* fb_lfs_readdir(DIR* dirp)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: readdir\n");
#  endif
  return (MPFS_READDIR(dirp));
}

/*
 * Does a closedir() call.
 */
static int fb_lfs_closedir(DIR* dirp)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: closedir\n");
#  endif
  return (MPFS_CLOSEDIR(dirp));
}

/*
 * Does an fstat of a file.
 */
static int fb_lfs_fstat(fb_fdesc_t* fd, struct stat64* statbufp)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: fstat\n");
#  endif
  return (MPFS_FSTAT(fd->fd_num, statbufp));
}

/*
 * Does a stat of a file.
 */
static int fb_lfs_stat(char* path, struct stat64* statbufp)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: stat:%s\n", path);
#  endif
  return (MPFS_STAT(path, statbufp));
}
...
{
#  ifdef MPFS_DEBUG
  printf("MPFS: pread:%d io_size:%d \n", fd->fd_num, iosize);
#  endif
  return (MPFS_PREAD64(fd->fd_num, iobuf, iosize, fileoffset));
}

/*
 * Does a posix read. Returns what the read() returns.
 */
static int fb_lfs_read(fb_fdesc_t* fd, caddr_t iobuf, fbint_t iosize)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: read:%d io_size:%d \n", fd->fd_num, iosize);
#  endif
  return (MPFS_READ(fd->fd_num, iobuf, iosize));
}
...
{
#  ifdef MPFS_DEBUG
  printf("MPFS: pwrite:%d iosize:%d\n", fd->fd_num, iosize);
#  endif
  return (MPFS_PWRITE(fd->fd_num, iobuf, iosize, offset));
}

/*
 * Do a write to a file.
 */
static int fb_lfs_write(fb_fdesc_t* fd, caddr_t iobuf, fbint_t iosize)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: write:%d iosize:%d\n", fd->fd_num, iosize);
#  endif
  return (MPFS_WRITE(fd->fd_num, iobuf, iosize));
}

/*
 * Does a truncate operation and returns the result
 */
static int fb_lfs_truncate(fb_fdesc_t* fd, off64_t fse_size)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: ftruncate\n");
#  endif

#  ifdef HAVE_FTRUNCATE64
  return (MPFS_FTRUNC(fd->fd_num, fse_size));
#  else
  filebench_log(LOG_ERROR, "Converting off64_t to off_t in ftruncate,"
                           " might be a possible problem");
  return (MPFS_FTRUNC(fd->fd_num, (off_t)fse_size));
#  endif
}
 ...
 */
static int fb_lfs_link(const char* existing, const char* new)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: link:%s-->%s\n", existing, new);
#  endif
  return (MPFS_LINK(existing, new));
}

/*
 * Does a symlink operation and returns the result
 */
static int fb_lfs_symlink(const char* existing, const char* new)
{
  printf("ERROR: symlink not implemented\n");
 ...
 */
static int fb_lfs_access(const char* path, int amode)
{
#  ifdef MPFS_DEBUG
  printf("MPFS: access:%s\n", path);
#  endif
  return (MPFS_ACCESS(path, amode));
}
#else
static int fb_lfs_open(fb_fdesc_t* fd, char* path, int flags, int perms)
{
