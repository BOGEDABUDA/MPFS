...
struct vm_operations_struct {
	...
	void (*map_pages)(struct vm_fault *vmf, pgoff_t start_pgoff,
			  pgoff_t end_pgoff);
...
#ifdef MPFS
	//User-space File System huge page fault
	uint32_t UFS_enable;
	vm_fault_t (*UFS_huge_fault)(struct vm_fault *vmf, pgd_t **pgd_pp);
#endif
	/*
...
static inline bool is_device_private_page(const struct page *page)
{
	return IS_ENABLED(CONFIG_DEV_PAGEMAP_OPS) &&
	       IS_ENABLED(CONFIG_DEVICE_PRIVATE) && is_zone_device_page(page) &&
...
{
	return IS_ENABLED(CONFIG_DEV_PAGEMAP_OPS) &&
	       IS_ENABLED(CONFIG_PCI_P2PDMA) && is_zone_device_page(page) &&
...
vma_merge(struct mm_struct *, struct vm_area_struct *prev, unsigned long addr,
	  unsigned long end, unsigned long vm_flags, struct anon_vma *,
	  struct file *, pgoff_t, struct mempolicy *,
...
do_mmap_pgoff(struct file *file, unsigned long addr, unsigned long len,
...
extern void filemap_map_pages(struct vm_fault *vmf, pgoff_t start_pgoff,
			      pgoff_t end_pgoff);
...
int drop_caches_sysctl_handler(struct ctl_table *, int, void __user *, size_t *,
			       loff_t *);
#endif
...
extern void clear_huge_page(struct page *page, unsigned long addr_hint,
