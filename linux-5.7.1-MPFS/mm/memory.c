...
#include "../drivers/dax/dax-private.h" // for MPFS
...
static inline unsigned long copy_one_pte(struct mm_struct *dst_mm,
...
again:
	...
	do {
		...
		entry.val = copy_one_pte(dst_mm, src_mm, dst_pte, src_pte, vma,
					 addr, rss);
		if (entry.val)
			break;
		progress += 8;
				 ...
				 unsigned long addr, unsigned long end)
{
	...
	do {
		...
		if (pmd_none_or_clear_bad(src_pmd))
			continue;
		if (copy_pte_range(dst_mm, src_mm, dst_pmd, src_pmd, vma, addr,
				   next))
			return -ENOMEM;
				 ...
				 unsigned long addr, unsigned long end)
{
	...
	do {
		...
		if (pud_none_or_clear_bad(src_pud))
			continue;
		if (copy_pmd_range(dst_mm, src_mm, dst_pud, src_pud, vma, addr,
				   next))
			return -ENOMEM;
				 ...
				 unsigned long addr, unsigned long end)
{
	...
	do {
		...
		if (p4d_none_or_clear_bad(src_p4d))
			continue;
		if (copy_pud_range(dst_mm, src_mm, dst_p4d, src_p4d, vma, addr,
				   next))
			return -ENOMEM;
				       ...
				       pgprot_t prot)
{
	...
	return err ? err :
		     insert_page_into_pte_locked(mm, pte_offset_map(pmd, addr),
						 addr, page, prot);
}
	...
	while (pages_to_write_in_pmd) {
		...
		for (; pte_idx < batch_size; ++pte_idx) {
			int err = insert_page_in_batch_locked(
				mm, pmd, addr, pages[curr_page_idx], prot);
			if (unlikely(err)) {
...
static vm_fault_t do_wp_page(struct vm_fault *vmf) __releases(vmf->ptl)
{
	...
	if (PageAnon(vmf->page)) {
		...
		if (PageKsm(vmf->page) &&
		    (PageSwapCache(vmf->page) || page_count(vmf->page) != 1))
			goto copy;
...
#ifdef MPFS
	if (vma->vm_ops != NULL &&
	    vma->vm_ops->UFS_enable == MPFS_UFS_ENABLED) {
		pgd = pgd_offset(mm, address);
		p4d = p4d_alloc(mm, pgd, address);
		if (!p4d)
			return VM_FAULT_OOM;

		// we must terminate original handle here and using mpfs_huge_fault to allocate pud, pmd, pte from main_task of MPFS, otherwise pud,pmd,pte will be allocated from different processes

		//actually mpfs_huge_fault()
		return vma->vm_ops->UFS_huge_fault(&vmf, &pgd);
	} else
		;
#endif
		       ...
		       unsigned int gup_flags)
{
	...
	while (len) {
		...
		ret = get_user_pages_remote(tsk, mm, addr, 1, gup_flags, &page,
					    &vma, NULL);
		if (ret <= 0) {
